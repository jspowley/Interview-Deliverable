...
```{r, forecasting independent variables}
# Model data rounds to the closest week. For more precision in forecasting to today's date, we're returning to core data used for each indicator to forecast today's value.

# US_Crude_Inventory
# Inspection of the US Crude graph reveals it's likely a random walk. Fortunately, this data stream is very frequent and up to date, making forecasting simpler and more accurate.

CI_ARMA <- fundamentals_data %>% 
  dplyr::filter(series == "US_Crude_Inventory") %>% 
  dplyr::filter(date >= start_date) %>% 
  arrange(date) %>% 
  ungroup() %>% 
  dplyr::select(value) %>% 
...
# High levels of autocorrelation justifies the use of integration / detrending. I expect this to be a random walk, so I integrate when ARMA modelling.
CI_ARMA %>% acf(main = "ACF Plot Data")
CI_ARMA %>% pacf(main = "PACF Plot Data")

US_CI_Fit <- CI_ARMA %>% 
  auto.arima()

# The following demonstrates the removal of autocorrelation from the data
US_CI_Fit$residuals %>% acf(main = "ACF Plot Residuals")
US_CI_Fit$residuals %>% pacf(main = "PACF Plot Residuals")

summary(US_CI_Fit)

# Failing to reject the null indicates the residuals resemble white noise. Although our model may not exhibit the most accuracy, we are content leaving it here since this tells us there's likely little we can do to improve accuracy beyond this point. (White Noise left over)
Box.test(US_CI_Fit$residuals, type = "Ljung-Box")

# The following conditional code is for interpolating between two forecast points or with todays data:
time_diff <- Sys.Date() - fundamentals_data %>% 
  dplyr::filter(series == "US_Crude_Inventory") %>% 
  dplyr::filter(date >= start_date) %>% 
  .$date %>% 
  max()
...
periodicity <- 7
forecast_end <- (time_diff/periodicity) %>% ceiling()

# US_Crude_Inventory_PF <- 0
if(forecast_end > 0){
  # When the report is not up to date, do the following forecast, and interpolate
  forecast <- forecast(US_CI_Fit, h = forecast_end) %>% as.data.frame()
  # print(forecast)
  forecast <- forecast %>% .$`Point Forecast`
  
  if(forecast_end == 1){
    start_value <- fundamentals_data %>% 
      dplyr::filter(series == "US_Crude_Inventory") %>% 
    ...
    forecast_ratio <- time_diff/periodicity
    
    US_Crude_Inventory_PF <- end_value * forecast_ratio + (1-forecast_ratio)*start_value
    
  } else {
    start_value <- forecast[forecast_end-1]
    end_value <- forecast[forecast_end]
    forecast_ratio <- time_diff/periodicity - floor(time_diff/periodicity)
    US_Crude_Inventory_PF <- end_value * forecast_ratio + (1-forecast_ratio)*start_value
  }
  
} else {
  # When the report is released today, use todays data point
  US_Crude_Inventory_PF <- fundamentals_data %>% 
    dplyr::filter(series == "US_Crude_Inventory") %>% 
    dplyr::filter(date == max(date)) %>% 
    .$value
}

US_Crude_Inventory_PF
```
```{r, generalized forecasting functions}
...
```{r, Forecasting Truck Tonnage}
# On quick observation, tonnage is nicely formatted as monthly data, without awkward gaps or compressions in the consistency of time reported.

# Observing our previous graph on truck tonnage, I believe there may by some structural component to the data which can be tackled using decomposition.
forecast_data <- fred_data %>% 
  dplyr::filter(series == "Truck_Tonnage_Index") %>% 
  arrange(date) %>% 
  ungroup()

arma_ready <- forecast_data %>% 
  dplyr::select(value) %>% 
  as.ts()

arma_ready %>% acf(main = "ACF Data")
arma_ready %>% pacf(main = "PACF Data")
# Autocorrelation visible, suggesting trend caused by p=1.
fit <- arma_ready %>% auto.arima()
# We auto-fit with p=1, q=0, as predicted. This process is auto-regressive.
summary(fit)
Box.test(fit$residuals, type = "Ljung-Box")
# Failing to reject the null, our residuals appear to be white noise, however is slightly more ambigious than in the crude case. (P Value of 0.32 means a true white noise process would have ~32% chance of producing what we observe in the residuals.)
fit$residuals %>% acf(main = "ACF of Residuals")
fit$residuals %>% pacf(main = "PACF of Residuals")

# Rough Approximation
periodicity <- 365/12
last_date <- forecast_data %>% dplyr::filter(date == max(date)) %>% .$date
time_diff <- (Sys.Date() - last_date) %>% as.numeric()

forecast_end
point_forecast <- forecast(fit, h = forecast_end) %>% as.data.frame() %>% .$`Point Forecast`
