---
title: "3-2-1 Crack Spread Recommendation"
author: "Justin Powley"
format: html
editor: visual
---

```{r, main_settings, echo = FALSE}
# Enable if rendering for printing. Will omit visuals from the main body to fit neatly within 2 pages.

# NOTE TO INTERVIEWER: Set this to true if you intend to print a hard copy. Some visuals will be excluded from the main body for visual clarity.

# Alternatively, set print size to 85% to fit up to two visuals per page.
printer_ready <- FALSE
```
...
```{r, echo = F}
# Preferred viewing format is as an HTML web page.

# Option to disable key charts within the written contents of the report. Helpful for tidy 2 page printing.
show_rc <- !printer_ready

# Alternatively, in print settings choose 80%-85% zoom to fit up to 2 visuals per page.

# Written contents are 2 pages
```
...
Historic data may have different equilibriums for the same fundamentals' levels. For example, technology can make production and refinement cheaper, which in turn means refineries are willing to produce more under less profitable conditions, which would drive the spread closer together. If necessary, shorter time frames can be used to train the regression model, while maintaining longer time frames for forecasting market driver. Another risk is that a market driver will respond to the high crack spread. This would result in a change in our prediction rather than in price.

With the previous factors in mind, assuming no reversals between dependent and independent variables, our model forecasts an **upside potential of `r upside_pot` per barrel.**
...
We use Value at Risk because it gives us a less biased risk estimate independent from our model. The **5% Value at Risk on 1 month positions in the crack spread is `r VAR`**. From this we can estimate that our best outcome in the worst 5% of cases is `r VAR` on a 1 month position. Tail risk is more extreme for short positions than long positions, supported by the data. A more detailed view of the monthly return distribution is displayed below and in the appendix:
...
To assess liquidity risk, the risk of getting closed out of a position due to higher daily volatility, see the rolling standard deviation chart below. It demonstrates both decreasing volatility over time, that volatility is currently lower than average, but also the tendency for volatility to spike for short periods. See the rolling volatility graphic.
...
### Sample ARIMA and STL Models

*Charts for STL and ARMA models exceed 45.* For brevity, only sample visualizations and stats used to assess model fit are included below. It outlines the diagnostic tools for each model. For a more detailed view of each model, go to the *Forecasting* code block and set show_all_charts <- True. Note that these charts will appear at the front of the report when rendered, since they are earlier in the workflow.

#### Sample ARIMA, Random Walk
