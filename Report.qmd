...
# Time difference between today and 
...
forecast_value <- forecast(US_CI_Fit, h = forecast_end) %>% as.data.frame() %>% .$`Point Forecast`
# A later idea for risk is to run lo and hi 95 bands into the model, to add systematic risk bands to it. (Kind of like sensitivity analysis)
# Not currently a necessary feature, but one which can be considered when more time is available.

forecast_ratio <- as.numeric(time_diff/(forecast_end*periodicity))

last_value <- fundamentals_data %>% 
  dplyr::filter(series == "US_Crude_Inventory") %>% 
  dplyr::filter(date == max(date)) %>% 
  .$value

# Interpolation between last data point and the forecasted data point, based on todays date
US_Crude_Inventory_PF <- forecast_value * forecast_ratio + (1-forecast_ratio)*last_value
```
