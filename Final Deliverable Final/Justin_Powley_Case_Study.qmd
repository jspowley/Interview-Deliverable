...
```{r, backtest, echo = F}
pl <- data_with_prediction %>% 
  dplyr::select(date, prediction, Crack_Spread) %>% 
  dplyr::mutate(long = prediction > Crack_Spread, yesterday_long = lag(long)) %>% 
  dplyr::mutate(profit_loss = case_when(yesterday_long ~ Crack_Spread - lag(Crack_Spread),
                                        TRUE ~ lag(Crack_Spread) - Crack_Spread)) %>% 
  drop_na()

pl_desc <- pl %>% summarise(Average_Profit_Loss = mean(profit_loss),
                 SD_Profit_Loss = sd(profit_loss),
                 Max_Monthly_Profit = max(profit_loss),
                 Max_Monthly_Loss = min(profit_loss))



pl_dist <- pl %>% ggplot() +
  geom_histogram(aes(x = profit_loss, y = ..count../sum(..count..)), bins = 50) +
  labs(title = "Backtested Profit and Loss, Per Crack Spread Barrel")
```
...
The 3-2-1 crack spread looks **`r recommendation2`** based on market fundamentals. Our model predicts a crack spread of **`r forecast_prediction %>% dollar()` using forecasted** values and **`r prediction_last_observed %>% dollar()` using the most recently observable** data points. Based on the most recent 3-2-1 crack spread **price of `r current_price %>% dollar()`**, a **`r recommendation` position** in the crack spread is recommended.
...
[Demand Side (Bullish Influence) Factors:]{style="font-weight: bold; font-size: 90%;"}
...
::: {.column width="50%" style="padding: 0; margin: 0;"}
[Supply Side (Bearish Influence) Factors:]{style="font-weight: bold; font-size: 90%;"}
...
Sometimes our market drivers will instead respond to the current crack spread. This results in our prediction moving instead of the market price. Assuming no reversals in dependency, our model forecasts an **upside potential of `r upside_pot %>% dollar()` per barrel**. As a point forecast, this number is likely overstated. Our **average upside potential based on back-testing is `r round(pl_desc$Average_Profit_Loss, digits = 2) %>% dollar()`** and our **standard deviation on PL is `r round(pl_desc$SD_Profit_Loss, digits = 2) %>% dollar()`**.
...
##### Backtesting Profit and Loss Summary (Per Contract Barrel):

```{r, backtesting, echo = FALSE}
pl_desc %>% kable()
pl_dist + labs(y = "Density", x = "PL per Contract Barrel")
```
